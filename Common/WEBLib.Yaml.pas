...
(*
  ...
  Reworked for TMS Web Core
  -------------------------
  09/2024 by Walter Weyers
  - issues resolved with javascript incompatible unicode constants
  - localisation issue when converting floats to string
...
uses
 System.SysUtils, System.Classes, System.Character, System.DateUtils, WEBLib.JSON,
...
type
 ...
 private
  ...
  class function InternalStringEscape(const AText: string): string;
  // Try convert a timestamp value to a datetime UTC
  class function InternalTryStrToDateTime(const AText: string; var ADate: TDateTime; AFormatSettings: TFormatSettings): boolean;
  // YAML to JSON section
  // --------------------
  // Read next text YAML line from strings
  class function InternalYamlNextTextLine(AYAML: TStrings; var ARow, AIndent: Integer; AIgnoreBlanks: boolean=True; AIgnoreComments: boolean=True; AAutoTrim: boolean=True): string;
  // Retrieve what next text will be
  class function InternalYamlNextText(AYAML: TStrings; var ARow, AIndent: Integer; AText: string; AIgnoreBlanks: boolean=True; AIgnoreComments: boolean=True; AAutoTrim: boolean=True): string;
  // Read next token from source, with multiline support
  class function InternalYamlReadToken(AYAML: TStrings; var ARow, AIndent: Integer; var AText, ARemainer, AAlias, ATag: string; var ACollectionItem: Integer; var AIsLiteral: boolean; AInArray: boolean=False): TYamlTokenType;
  // Find element containing an anchor by anchor name
  class function InternalYamlFindAnchor(AElements: TYamlElements; AAnchorName: string): Integer;
  ...
  class procedure InternalYamlProcessArray(AYAML: TStrings; AElements: TYamlElements; var ARow, AIndent: Integer; var AText: string; AYesNoBool: boolean; AAllowDuplicateKeys: boolean);
  // Process a collection (array) from source
  class procedure InternalYamlProcessCollection(AYAML: TStrings; AElements: TYamlElements; var ARow, AIndent: Integer; var AText: string; AYesNoBool: boolean; AAllowDuplicateKeys: boolean);
  // Process element pairs (key: value) from source
  class procedure InternalYamlProcessElements(AYAML: TStrings; AElements: TYamlElements; var ARow, AIndent: Integer; var AText: string; AYesNoBool: boolean; AAllowDuplicateKeys: boolean);
  // Format a YAML value to JSON
  class function InternalYamlProcessJsonValue(AValue: string; ALiteral: boolean; ATag: string; ALineNumber: Integer; AYesNoBool: boolean): string;
  ...
  class procedure InternalJsonObjToYaml(AJSON: TJSONObject; AOutStrings: TStrings; AIndentation: TYamlIdentation; var AIndent: Integer; AFromArray: boolean=False; AYesNoBool: boolean=False);
  // Process JSON array to YAML
  class procedure InternalJsonArrToYaml(AJSON: TJSONArray; AOutStrings: TStrings; AIndentation: TYamlIdentation; var AIndent: Integer; AFromArray: boolean=False; AYesNoBool: boolean=False);
  // Convert a value from JSON to YAML
  class function InternalJsonValueToYaml(AJSON: TJSONValue; AIndent: Integer=0; AYesNoBool: boolean=False): TArray<string>;
 public
  // JSON to YAML
  class function JsonToYaml(AJSON: string; AIndentation: TYamlIdentation=2; AYesNoBool: boolean=False): string; overload;
  ...
  class function JsonToYaml(AJSON: TJSONValue; AIndentation: TYamlIdentation=2; AYesNoBool: boolean=False): string; overload;
  class procedure JsonToYaml(AJSON: TJSONValue; AOutStrings: TStrings; AIndentation: TYamlIdentation=2; AYesNoBool: boolean=False); overload;
  // YAML to JSON
  ...
  class function YamlToJson(AYAML: string; AIndentation: TJsonIdentation=2; AYesNoBool: boolean=True; AAllowDuplicateKeys: boolean=True): string; overload;
  class function YamlToJson(AYAML: TStrings; AIndentation: TJsonIdentation=2; AYesNoBool: boolean=True; AAllowDuplicateKeys: boolean=True): string; overload;
  class function YamlToJsonValue(AYAML: string; AIndentation: TJsonIdentation=0; AYesNoBool: boolean=True; AAllowDuplicateKeys: boolean=True): TJSONValue; overload;
  class function YamlToJsonValue(AYAML: TStrings; AIndentation: TJsonIdentation=0; AYesNoBool: boolean=True; AAllowDuplicateKeys: boolean=True): TJSONValue; overload;
  // JSON tools
  class function JsonMinify(AJSON: string): string; overload;
  class function JsonMinify(AJSON: TStrings): string; overload;
 end;
...
begin
 ...
 //   LText := LText.Replace(#$2028, '\u2028')
 // this crashes js and/or the browser! Wrapping them in Chr() fixes this.

 C := Chr($2028); LText := LText.Replace(C, '\u2028'); // line separator (unicode)
 C := Chr($0085); LText := LText.Replace(C, '\u0085'); // new line (unicode)
 C := Chr($2029); LText := LText.Replace(C, '\u2029'); // paragraph separator (unicode)
...
begin
 ...
 if TryStrToInt(LText.Substring(0, 1), LNumber) then
  begin
   ...
   Result := System.DateUtils.TryRFC3339ToDateTime(LText, ADate);
   {$ENDIF}
      ...
      if LElement.Key.IsEmpty then
       ...
       begin
        ...
        if LElement.Tag=LTagBin then
         begin
          LBase64Decoder := System.NetEncoding.TNetEncoding.Base64;
          try
           try
            // WAS: LBytes := LBase64Decoder.DecodeStringToBytes(LValue);
            LBytes := TEncoding.UTF8.GetBytes(LBase64Decoder.Decode(LValue));
           except
            raise EYamlParsingException.CreateFmt(EYamlInvalidValueForTagError, [LElement.LineNumber]);
           end;
 ...
 if (AJSON is TJSONFalse) then
  ...
  begin
   ...
   if Frac(LFloat)=0 then
    ...
    LText := FloatToStr(LFloat, TFormatSettings.Create('en-US'))
  end
...
begin
 LJSONValue := nil;
 try
  LJSONValue := TJSONObject.ParseJSONValue(AJSON.Text);
 except
...
begin
 ...
 Result := TJSONObject.ParseJSONValue(LJSON);
end;
